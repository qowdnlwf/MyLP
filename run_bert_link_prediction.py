...
# Deprecated pytorch_pretrained_bert
...
def main():
    ...
    # Prepare model
    cache_dir = args.cache_dir if args.cache_dir else os.path.join(str(os.getenv('TRANSFORMERS_CACHE', './cache')), 'distributed_{}'.format(args.local_rank))
    # model = BertForSequenceClassification.from_pretrained(args.bert_model,
    #           cache_dir=cache_dir,
    #           num_labels=num_labels)
    model = AutoModelForSequenceClassification.from_pretrained(args.bert_model, cache_dir=cache_dir,num_labels=num_labels)
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        all_triples_str_set = set('\t'.join(triple) for triple in all_triples)
        ...
        for test_triple in test_triples:
            ...
            file_prefix = f"{args.data_dir[7:]}_{args.train_batch_size}_{args.learning_rate}_{args.max_seq_length}_{args.num_train_epochs}"
            with open(file_prefix + '_ranks.txt', 'a') as f:
                f.write(f"{rank1}\t{rank2}\n")
            # this could be done more elegantly, but here you go
            for hits_level in range(10):
                hit_value_rank1 = 1.0 if rank1 <= hits_level else 0.0
                hits[hits_level].append(hit_value_rank1)
                hits_left[hits_level].append(hit_value_rank1)
        ...
        logger.info(f"Mean reciprocal rank left: {np.mean(1.0 / np.array(ranks_left))}")
        logger.info(f"Mean reciprocal rank right: {np.mean(1.0 / np.array(ranks_right))}")
        logger.info(f"Mean reciprocal rank: {np.mean(1.0 / np.array(ranks))}")
